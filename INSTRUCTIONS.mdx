...
Pour créer des relations en SQL, il faut plusieurs tables contenant chacune une `PK (Primary Key)`. La liaison se fait via une clef étrangère, `FK (Foreign Key)`. On utilise `REFERENCES` pour définir la `FK`. Exemple :
...
- Définir les Relations (Au niveau applicatif avec `relation`)
...
**🐶** Dans cet exercice, tu vas devoir créer une relation 1-1 entre `USER` et `PROFILEINFO`
...
La puissance de l’`ORM` réside également dans le fait que lorsque l’on définit correctement les relations, il est très simple d’accéder aux données ainsi que les données associées au lieu d’avoir à faire plusieurs requêtes SQL séparées ou une grosse requête complexe avec des jointures.

**🐶** Dans cet exercice bonus, tu vas appeler `USER` et demander d’avoir le profil avec pour afficher la note.
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=06.Data%20Modeling&entry.533578441=06%20Relation%20One%20To%20One).
