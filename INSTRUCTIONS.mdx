...
Il est important d’avoir une codebase en phase avec la modélisation de notre base de données. Dans les modules précèdent nous avions

- Des Scripts SQL pour créer notre model de données
- Notre code base

S'assurer que tout est en phase peut être fastidieux et l’on est jamais à l’abris d’un oublie. Dans une application il y à gérer le développement de nouvelle `feature` mais aussi l’évolution.
...
📑 Le liens vers la doc : [https://orm.drizzle.team/kit-docs/overview#overview](https://orm.drizzle.team/kit-docs/overview#overview)
...
**🐶** Pour que ces scripts fonctionnent tu vas creer un fichier de configuration drizzle
...
Il est intéressant de voir que l’on peux :

- Exécuter des requêtes `SQL`
- Exécuter des requêtes `Drizzle`
- Introspecter les tables dans l’onglet `shema`

Créer une nouvelle table `TOTO` et constate l’introspection
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=Data%20Modeling&entry.533578441=05%20Drizzle%20Kit).
